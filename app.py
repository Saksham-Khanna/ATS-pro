...
submit3 = st.button("Suggest Missing Sections with improvements needed")
submit4 = st.button("Keyword Gap Analysis")
submit5 = st.button("Bias Detection")
...
input_prompt4 = """
Compare the job description and the resume. Identify the important skills, tools, or keywords mentioned in the job description that are missing from the resume. List these missing keywords clearly, and suggest how the candidate can naturally include them in their resume if they actually have those skills.
"""

input_prompt5 = """
You are an expert in identifying biases in resumes.Analyze the resume text and identify elements that may cause bias or reduce ATS-friendliness. Look for:
Personal details like age, gender, marital status, religion, father's name, or nationality.
Non-ATS-friendly formatting such as photos, tables, or graphics.
Unnecessary personal information that does not add professional value.
List each issue found and provide a recommendation on how to fix it in a professional and ATS-friendly way.
"""
